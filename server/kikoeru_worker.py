...
def acquireTask(url:str)->Tuple[bool, bool, Dict]:
    try:
        res = session.post(
            f"{url}/drama/lyric/translate/acquire",
            {
...
def updateTaskStatus(task:Dict, status:str)->bool:
    try:
        res = session.post(
            f"{kikoeru_url}/drama/lyric/translate/status",
            {
                **task,
...
def downloadAudioFile(task:Dict, save_name:str)->bool:
    try:
        url = session.get(f"{kikoeru_url}/drama/lyric/translate/download", params={
            "id": task["id"],
            "secret": task["secret"],
        }).text.replace('"','')
...
def finishTask(task:Dict, success:bool, lrc_content:str):
    print("上传任务结果")
    try:
        r = session.post(f"{kikoeru_url}/drama/lyric/translate/finish", json={
            "id": task["id"],
            "secret": task["secret"],
            "success": success,
...
def checkTaskIsOwnByMe(task:Dict)->bool:
    try:
        r = session.get(f"{kikoeru_url}/drama/lyric/translate/get", params={
            "id": task["id"],
            "secret": task["secret"],
        })
        if r.status_code == 200:
